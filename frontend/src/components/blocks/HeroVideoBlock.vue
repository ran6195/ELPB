<template>
  <div class="hero-block" :class="{ 'max-w-7xl mx-auto': block.content.backgroundContained }" :style="wrapperStyles">
    <!-- Layer 1: video di sfondo -->
    <video
      v-if="block.content.backgroundVideo"
      :style="bgVideoStyles"
      autoplay
      loop
      muted
      playsinline
    >
      <source :src="block.content.backgroundVideo" :type="getVideoType(block.content.backgroundVideo)">
    </video>
...
const bgVideoStyles = computed(() => ({
  ...
  height: '100%',
  objectFit: 'cover',
  opacity: props.block.content.backgroundVideoOpacity ?? 1
}))
