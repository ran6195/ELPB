...
### Sessione 2026-06-28
- **Colore Contatti Blocco Mappa** 🎨: Aggiunto campo `contactColor` al blocco Mappa per scegliere il colore di indirizzo, telefono ed email (prima sempre grigio/nero hardcoded `text-gray-600`). Applicato a label `<h4>` e valori `<p>`, con `<a>` tel/mailto che ereditano (`color:inherit`). Controllo colore nell'editor sotto i campi contatto, computed `contactStyles` nel componente Vue, variabile `$contactStyle` nei due renderer (MapBlock.vue, BlockEditor.vue, standalone BlockRenderer.php, joomla blockrenderer.php)
