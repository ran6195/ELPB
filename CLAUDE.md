...
### Sessione 2026-06-28
- **Colore Contatti Blocco Mappa** 🎨: Aggiunto campo `contactColor` al blocco Mappa per scegliere il colore di indirizzo, telefono ed email (prima sempre grigio/nero hardcoded `text-gray-600`). Applicato a label `<h4>`, valori `<p>` e icone SVG (`stroke="currentColor"` + `color`, prima `text-primary-600` fisso), con `<a>` tel/mailto che ereditano (`color:inherit`). Controllo colore nell'editor sotto i campi contatto, computed `contactStyles` nel componente Vue, variabile `$contactStyle` nei due renderer (MapBlock.vue, BlockEditor.vue, standalone BlockRenderer.php, joomla blockrenderer.php)
- **Ombra sui Titoli dei Blocchi** ✨: Aggiunta opzione **per-blocco** e completamente personalizzabile (colore, sfocatura, offset X/Y) per applicare un'ombra (text-shadow) al titolo. Utility condivisa `titleShadowStyle()` (utils/titleStyle.js) e componente riusabile `TitleShadowEditor.vue` mostrato nell'editor solo per i blocchi con titolo (computed `blockHasTitle` in BlockEditor.vue, sezione condivisa unica). Lato renderer iniettato centralmente in `buildTitleStyle()` (nuovo param `$titleShadow` + helper `buildTitleShadowCss`), quindi copre tutti i 14 blocchi con titolo. Merge di `titleShadowStyle(block.content)` nello `:style` del titolo dei 14 componenti Vue (Hero/HeroWide/HeroVideo, Text, Features, ServicesGrid, Cta, TwoColumn×2, VideoInfo, Form, FormAvanzato, Slider, Map). Campo `titleShadow` in content `{enabled,color,blur,offsetX,offsetY}` (titleStyle.js, TitleShadowEditor.vue, BlockEditor.vue, 14 blocchi Vue, standalone BlockRenderer.php, joomla blockrenderer.php)
