...
class BlockRenderer
{
     ...
     */
    protected function buildTitleStyle($titleColor, $titleSize = '', $titleShadow = null)
    {
        ...
        $css .= $this->buildTitleShadowCss($titleShadow);
     ...
     */
    protected function buildTitleShadowCss($titleShadow)
    {
...
HTML;
    ...
    protected function renderFeatures($content, $styles, $block)
    {
        ...
        $titleStyle = $this->buildTitleStyle($effectiveTitleColor, $content['titleSize'] ?? '', $content['titleShadow'] ?? null);
...
HTML;
    ...
    protected function renderServicesgrid($content, $styles, $block)
    {
        ...
        $roundedClass = $this->getRoundedClass();
        $blockStyle = $this->getBlockStyle($styles);
        $titleStyle = $this->buildTitleStyle('', $content['titleSize'] ?? '', $content['titleShadow'] ?? null);
...
HTML;
    ...
    protected function renderVideoinfo($content, $styles, $block)
    {
        ...
        if ($youtubeId && !$isShort) {
            $embedUrl = htmlspecialchars($this->getEmbedUrl($content['videoUrl'], $autoplay, $loop, $muted, $showControls, $playOnScroll));
            $videoHtml = <<<HTML
        ...
        if ($showContactInfo && (!empty($address) || !empty($phone) || !empty($email))) {
            ...
            if (!empty($address)) {
                ...
                <div class="flex items-start gap-3">
                    <svg class="w-6 h-6 text-primary-600 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"{$contactStyle}>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                    </svg>
                    <div>
